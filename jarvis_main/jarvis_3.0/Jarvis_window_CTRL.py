...
APP_MAPPINGS = {
    ...
    "postman": "C:\\Users\\Lenovo\\AppData\\Local\\Postman\\Postman.exe",
    "whatsapp": "start whatsapp:",  # ✅ works for Microsoft Store WhatsApp
...
async def open_app(app_title: str) -> str:
    """
    Open a desktop app or website like Notepad, Chrome, WhatsApp, or YouTube.
    """
    ...
    try:
        if app_command.startswith("http") or app_command.startswith("https"):
            # ✅ For websites
            os.startfile(app_command)
            return f"🌐 Website opened: {app_title}"
        elif app_command.startswith("start "):
            # ✅ For URI schemes like start ms-settings: or start whatsapp:
            subprocess.Popen(["cmd", "/c", app_command], shell=True)
            return f"🚀 App launched: {app_title}"
        else:
            # ✅ For normal EXE paths or built-in apps
            subprocess.Popen(["cmd", "/c", "start", "", app_command], shell=True)
            return f"🚀 App launched successfully: {app_title}"

    except Exception as e:
        return f"❌ {app_title} Launch नहीं हो पाया। Error: {e}"
...
async def close_app(window_title: str) -> str:
    """
    Closes the application's window by its title.
    """
    if not win32gui:
...
async def folder_file(command: str) -> str:
    """
    Handles folder and file actions like open, create, rename, or delete.
    """
    folders_to_index = [
